...
def NMAP_SSL(nmap_files_location, output_location, Dict_System = {}, Array_Filter_Services  = ["access denied", ""], Array_Temp = []):
        try:
                # Check_For_One_File
                            ...
                            for event, elem in ET.iterparse(nmap_files_location, events=("end",)):
                                if (event == "end"):
                                    if (elem.tag == "address"):
                                        ...
                                        if (elem.attrib['id'] == 'ssl-enum-ciphers'):
                                            Array_Temp_Ciphers = resplit("\n", elem.attrib['output'])
                                            for _ in range(0, len(Array_Temp_Ciphers)):
                                                if ('SSLv2'   in Array_Temp_Ciphers[_] or
                                                    'SSLv3'   in Array_Temp_Ciphers[_] or
                                                    'TLSv1.0' in Array_Temp_Ciphers[_] or
                                                    'TLSv1.1' in Array_Temp_Ciphers[_] or
                                                    'TLSv1.2' in Array_Temp_Ciphers[_] or
                                                    'TLSv1.3' in Array_Temp_Ciphers[_]):
                                                        SSL_Protocol = Array_Temp_Ciphers[_][:-2][1:]

                                                if ('ciphers' in Array_Temp_Ciphers[_]):
                                                    _ += 1
                                                    try:
                                                        while ('compressors' not in Array_Temp_Ciphers[_] and
                                                               'warnings'    not in Array_Temp_Ciphers[_] and
                                                        'cipher preference:' not in Array_Temp_Ciphers[_] and
                                                           'least strength:' not in Array_Temp_Ciphers[_]):
                                                                    print (f'{Address}:{Port} {SSL_Protocol} {Array_Temp_Ciphers[_][6:-4].split(" ")[0]} {Array_Temp_Ciphers[_][6:-4].split("(")[1][:-1].replace(" ","_")}')
                                                                    _ += 1
