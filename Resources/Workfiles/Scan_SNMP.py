...
class Check_SNMP:
    def Basic_Check(url, t_seconds, Host_Name, Location, Dict_System = {}):
        ...
        for oid in Array_OIDs:
            # SNMPv1
            Temp_Output = Walk_SNMP(oid, Target, 0, 'Public')

            # SNMPv2
            if (Temp_Output == None):
                Temp_Output = Walk_SNMP(oid, Target, 1, 'Public')
    ...
    def String_Enumeration(url, t_seconds, Host_Name, Location, Dict_System = {}):
        ...
        for _ in Array_Community_Strings:        
            # SNMPv1
            Temp_Output = Walk_SNMP('iso.3.6.1.2.1.1.5.0', Target, 0, _)

            # SNMPv2
            if (Temp_Output == None):
                Temp_Output = Walk_SNMP('iso.3.6.1.2.1.1.5.0', Target, 1, _)
