...
def SSH_Vulns(url, t_seconds, Host_Name, Location, Dict_System = {}):
    filter_values    = r'x88|x01|x00|x15|xd5|\\|&|@|none|x03|x0c|x14|x9f|x98|x7fl|xb4|xe6o|xa7|xee|x1b|xd8|x8c|x034|x92|~|x0f|x9d|xb8{|xf4|xe7|xcc|x89$Q|xef|xaa|xea|x93^|xe3|t:"F|/|openssh.com|xd1b+|xa0|xe8|xd9|x85|xba|xbf|lysator.liu.se|libssh.org'
                                    ...
                                    'pgp'   in cipher):
                                          if (cipher[0].isdecimal()):
                                              if (cipher not in Dict_SSH_Results['server_host_key_algorithms']):
                                                  Dict_SSH_Results['server_host_key_algorithms'].append(cipher[1:])
                                          else:
                                              if (cipher not in Dict_SSH_Results['server_host_key_algorithms']):
    ...
    try:
        ...
        if (Host_Name != ""):
            Logs.Log_File(
                ...
                +Colors.YELLOW+'-----------------------------------------------------------------------------------------------------------\n'
                +Colors.GREEN+f'{strftime("%Y-%m-%d %H:%M:%S")}'+Colors.RESET+f' - {Target}:{Port} - {Host_Name}'
                +Colors.BLUE+'\n-----------------------------------------------------------------------------------------------------------'
            ...
            Logs.Log_File(
                ...
                +Colors.BLUE+'SSH-Check\n'
                +Colors.YELLOW+'-----------------------------------------------------------------------------------------------------------\n'
                +Colors.GREEN+f'{strftime("%Y-%m-%d %H:%M:%S")}'+Colors.RESET+f' - {Target}:{Port}'
                +Colors.BLUE+'\n-----------------------------------------------------------------------------------------------------------'
                +Colors.ORANGE+'\nOriginal Output'+Colors.RED+' -> '+Colors.RESET+f'{Temp_Filter}'
                +Colors.BLUE+'\n-----------------------------------------------------------------------------------------------------------'
                +Colors.ORANGE+'\nEinherjer Filter'+Colors.RED+' -> '+Colors.RESET+f'{Dict_SSH_Results}\n\n',
